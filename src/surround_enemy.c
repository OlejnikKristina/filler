...
** shold go according to game position
**
...
int		target_top(t_map *map, char game_chr, bool game_from_right)
{
	short		x;
	short		y;
...
void	top(t_game *game, t_map *map, int *y, int *x)
{
	int static		pre_x = 0;
	int static		pre_y = 0;
...
{
	if ((game->figure_view == 'h' && !game->hit_right) || game->hit_bottom)
	{
